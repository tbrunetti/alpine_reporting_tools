...
do
	...
	echo "NEW USER: ${username}" >> SU_${END_DATE}.txt
	# report the total number of SUs consumed by each user
        echo "SU" >> SU_${END_DATE}.txt
        sreport  -t hours -T billing -P cluster AccountUtilizationByUser users=${username} start=${START_DATE} end=${END_DATE} tree acct=amc-general | sed -n '5,6p' >> SU_${END_DATE}.txt
done
